...
class Custom_RPNHead(RPNTestMixin, AnchorHead):
    """RPN head.
    ...
    def __init__(self, in_channels, **kwargs):
        super(Custom_RPNHead, self).__init__(1, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.rpn_conv_mask = nn.Conv2d(1, 1, 3, 1)
        self.rpn_reg_mask = nn.Conv2d(1, self.num_anchors * 4, 1)
        
    def init_weights(self):
        """Initialize weights of the head."""
        normal_init(self.rpn_conv_mask, std=0.01)
    ...
    def forward_single(self, x, mask=None):
        ...
        if mask == None:
            ...
            # print(mask.shape)
            # mask = self.rpn_conv_mask(mask)
            # mask = F.relu(mask, inplace=True)
            rpn_bbox_pred = self.rpn_reg_mask(mask)
            # torch.Size([2, 12, 190, 334])
            # torch.Size([2, 12, 192, 336])
            # print(rpn_bbox_pred.shape)
    ...
    def loss(self,
        ...
        """
        losses = super(Custom_RPNHead, self).loss(
            cls_scores,
            bbox_preds,
            gt_bboxes,
            None,
    ...
    def forward(self, feats, label_mask=None):
        ...
        if label_mask == None:
            ...
            return multi_apply(self.forward_single, feats, label_mask)


    def simple_test_rpn(self, x, img_metas, label_mask=None):
    ...
    def forward_train(self,
        ...
        """
        outs = self(x, label_mask) # 就是执行__call__ 函数
