...
class graph_encoder(torch.nn.Module):
    def __init__(self, embed_dims):
        ...
        for j in range(6):
            self.encoder += [
                Seq(Grapher(embed_dims, 9, 1, 'mr', 'gelu', 'batch',
                                n=196, drop_path=0.0,relative_pos=True),
                        FFN(embed_dims, embed_dims * 4, act='gelu', drop_path=0.0)
                        )]
        self.encoder = Seq(*self.encoder)

        self.pos_embed = nn.Parameter(torch.zeros(1, embed_dims, 14, 14))
        self.model_init()
