...
class VisionEncoder(torch.nn.Module):
    def __init__(self,encoder_output_dim=256):
        ...
        # conv_5
        self.conv_5 = torch.nn.Sequential(torch.nn.Conv2d(512,1024,kernel_size = 3 , stride = 1, padding=1),
                                        torch.nn.ReLU(True))
        self.adp_pool_5 = torch.nn.AdaptiveAvgPool2d((1,1))
        self.linear_proj_5 = torch.nn.Sequential(torch.nn.Linear(1024,encoder_output_dim),
                                                torch.nn.ReLU(True))
...
class GatedAttentionDIP(torch.nn.Module):
    """_summary_
    ...
    def __init__(self,
        ...
        """
        super(GatedAttentionDIP,self).__init__()
        ...
        # Gating Module
        self.gate_module = torch.nn.Sequential(torch.nn.Linear(encoder_output_dim,num_of_gates * 3,bias=True))
    ...
    def defog(self,x:torch.tensor ,latent_out : torch.tensor ,fog_gate : torch.tensor):
        ...
        j = j* fog_gate.unsqueeze(2).unsqueeze(3)
        return j
    ...
    def white_balance(self,x : torch.tensor,latent_out : torch.tensor ,wb_gate: torch.tensor):
        ...
        wb_out = wb_gate.unsqueeze(2).unsqueeze(3)*wb_out
        return wb_out
    ...
    def gamma_balance(self,x : torch.tensor,latent_out : torch.tensor,gamma_gate : torch.tensor):
        ...
        g = g*gamma_gate.unsqueeze(2).unsqueeze(3)
        return g
    ...
    def sharpning(self,x : torch.tensor,latent_out: torch.tensor,sharpning_gate : torch.tensor):
        ...
        s = s * (sharpning_gate.unsqueeze(2).unsqueeze(3))
        return s
    ...
    def identity(self,x : torch.tensor,identity_gate : torch.tensor):
        ...
        Returns:
            _type_: _description_
        """
        x = x*identity_gate.unsqueeze(2).unsqueeze(3)
        return x
    ...
    def contrast(self,x : torch.tensor,latent_out : torch.tensor,contrast_gate : torch.tensor):
        ...
        contrast_image = contrast_image * contrast_gate.unsqueeze(2).unsqueeze(3)
    ...
    def tone(self,x : torch.tensor,latent_out : torch.tensor,tone_gate : torch.tensor):
        ...
        total_image = total_image * tone_gate.unsqueeze(2).unsqueeze(3)
        return total_image
    ...
    def forward(self, x : torch.Tensor):
        ...
        en_x = nn.functional.interpolate(x, scale_factor=0.25, mode='bilinear', align_corners=False)
        latent_out = self.encoder(en_x)
        
        gate = self.tanh_range(self.gate_module(latent_out),0.01,1.0)
        # gate = gate.unsqueeze(2).unsqueeze(3)
        wb_out = self.white_balance(x,latent_out,gate[:, :3])
        gamma_out = self.gamma_balance(x,latent_out,gate[:, 3:6])
        identity_out = self.identity(x,gate[:, 6:9])
        sharpning_out = self.sharpning(x,latent_out,gate[:, 9:12])
        fog_out = self.defog(x,latent_out,gate[:, 12:15])
        contrast_out = self.contrast(x,latent_out,gate[:, 15:18])
        tone_out = self.tone(x,latent_out,gate[:, 18:21])
        # print(wb_out.shape, gamma_out.shape, fog_out.shape)
        # print(sharpning_out.shape, contrast_out.shape, tone_out.shape, identity_out.shape)
        # print(gate[:, :3].shape)
        x = wb_out + gamma_out   + fog_out + sharpning_out + contrast_out + tone_out + identity_out
        # x = (x-x.min())/(x.max()-x.min()) # 3050的结果没有使用归一化
        
        return x,gate


class GatedAttentionDIP_wodefog(GatedAttentionDIP):
    """_summary_
    ...
    def __init__(self,
        ...
        Args:
            ...
            num_of_gates (int, optional): _description_. Defaults to 7.
        """
        super(GatedAttentionDIP_wodefog,self).__init__()
        
        self.gate_module = torch.nn.Sequential(torch.nn.Linear(encoder_output_dim, 18,bias=True))
    ...
    def forward(self, x : torch.Tensor):
        ...
        identity_out = self.identity(x,gate[:, 6:9])
        sharpning_out = self.sharpning(x,latent_out,gate[:, 9:12])
        tone_out = self.tone(x,latent_out,gate[:, 12:15])
        contrast_out = self.contrast(x,latent_out,gate[:, 15:18])
        # tone_out = self.tone(x,latent_out,gate[:, 18:21])
        # print(wb_out.shape, gamma_out.shape, fog_out.shape)
        # print(sharpning_out.shape, contrast_out.shape, tone_out.shape, identity_out.shape)
        # print(gate[:, :3].shape)
        x = wb_out + gamma_out + sharpning_out + contrast_out + tone_out + identity_out
...
if __name__ == '__main__':
    ...
    x = (x-x.min())/(x.max()-x.min())
    model = GatedAttentionDIP(encoder_output_dim = encoder_out_dim).cuda()
    print(model)
    out,gate= model(x)
